/**
 * Client-side barcode lookup via Open Food Facts API.
 * OFF serves CORS headers (access-control-allow-origin: *),
...
function toImperial(qty: number, unit: string): { qty: number; unit: string } {
  if (unit === 'ml') {
    ...
    const rounded = Math.round(floz * 2) / 2;
  ...
  if (unit === 'L') return toImperial(qty * 1000, 'ml');
  if (unit === 'g') {
    const oz = qty / G_PER_OZ;
    if (oz >= 16) return { qty: Math.round(oz / 16 * 10) / 10, unit: 'lb' };
    const rounded = Math.round(oz * 2) / 2;
    return { qty: rounded || 1, unit: 'oz' };
  }
  if (unit === 'kg') return toImperial(qty * 1000, 'g');
  return { qty, unit };
}
...
export async function lookupBarcode(code: string): Promise<BarcodeResult> {
  ...
  if (!name) throw new Error('Product found but name is missing');

  const brand = product.brands?.split(',')[0]?.trim();
  const category = mapCategory(product.categories_tags?.join(' ') ?? '');
  ...
  if (product.product_quantity_unit) {
    unit = unitMap[product.product_quantity_unit.toLowerCase()];
  }
