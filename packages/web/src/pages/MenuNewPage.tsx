...
const CREATE_MUTATION = `mutation(
  $title: String!, $description: String, $active: Boolean, $category: String,
  $recipes: [MenuRecipeInput!]!
) {
  createMenu(
    title: $title, description: $description, active: $active, category: $category,
    recipes: $recipes
  ) { id slug }
}`;
...
export default function MenuNewPage() {
  ...
  // Group recipes by inferred course
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const { createMenu } = await gql<{ createMenu: { slug: string } }>(CREATE_MUTATION, {
        title: title.trim(),
      ...
      navigate(`/menus/${createMenu.slug}#stage`);
    } catch (err) {
      console.error(err);
      setSaving(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
        <div>
          ...
          <input
            ...
            placeholder="e.g. Dinner Party, This Week's Meals"
          />
        </div>
