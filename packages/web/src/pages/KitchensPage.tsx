...
const CREATE_KITCHEN = `mutation CreateKitchen($slug: String!, $name: String!) { createKitchen(slug: $slug, name: $name) { id slug name } }`;
const UPDATE_KITCHEN = `mutation UpdateKitchen($id: String!, $name: String!) { updateKitchen(id: $id, name: $name) { id slug name } }`;
const DELETE_KITCHEN = `mutation DeleteKitchen($id: String!) { deleteKitchen(id: $id) }`;
    ...
    <div>
          ...
          <form onSubmit={handleCreate} noValidate>
            ...
            <div className="mb-5">
              <label htmlFor="kitchen-slug" className="field-label">
                Slug <span className="font-normal text-[var(--color-text-secondary)]">(URL-friendly)</span>
              </label>
              <input
                id="kitchen-slug"
                type="text"
                ...
                placeholder="e.g. grandmas-house"
                pattern="[a-z0-9-]+"
                className="field-input w-full font-mono"
                aria-describedby="slug-hint"
              />
              <p id="slug-hint" className="mt-1 text-xs text-[var(--color-text-secondary)]">
                Used in URLs like <code>/kitchens/{slug || 'grandmas-house'}/recipes</code>
              </p>
            </div>
            {error && <p role="alert" className="mb-3 text-sm text-red-500">{error}</p>}
            <div className="flex gap-3">
              <button type="submit" disabled={saving || !name.trim() || !slug.trim()} className="btn-primary disabled:opacity-50">
      ...
      <ul role="list" className="space-y-4">
        {kitchens.map((k) => {
          ...
          return (
            <li key={k.id} className="card rounded-xl p-5">
                ...
                {isEditing ? (
                  <form onSubmit={handleRename} className="flex-1 flex gap-2 items-center flex-wrap" noValidate>
                    <input
                      ...
                      autoFocus
                    />
                    {editError && <p role="alert" className="w-full text-sm text-red-500">{editError}</p>}
                    <button type="submit" disabled={editSaving || !editName.trim()} className="btn-primary text-sm disabled:opacity-50">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isHome && <span className="text-xs font-medium text-[var(--color-accent)]">default</span>}
              ...
              {!isEditing && (
                <div className="flex flex-wrap gap-2">
                  <Link to={`${base}/recipes#stage`} className="btn-secondary text-sm">Recipes</Link>
                  <Link to={`${base}/ingredients#stage`} className="btn-secondary text-sm">Pantry</Link>
                  <Link to={`${base}/list#stage`} className="btn-secondary text-sm">List</Link>
                  <Link to={`${base}/cookware#stage`} className="btn-secondary text-sm">Cookware</Link>
                </div>
              )}
            </li>
          );
